...
Edit the file <code>deployment.properties</code> in the root of the project. Here is an example <code>deployment.properties</code> file for an AWS EKS environment; see the examples folder [here](deployment-properties-examples) for an Azure AKS example:
